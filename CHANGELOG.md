...
- `deep-diffs-wiki` CLI — renders a live Wikipedia article's revision history (MediaWiki Action API, rate-limit friendly) as a deep-diff report, including an author-coloured lens over the top contributors — the paper's original corpus, revisited. Wikitext stripping runs in linear time even on adversarial input (quadratic regex scans found in review and replaced with indexOf-based passes); vandalism/revert cycles are collapsed out of the revision timeline before sampling (History Flow's collapse rule, `--keep-reverts` to disable) and the Recency lens buckets by real calendar date rather than sample rank, so a heavily-vandalised or long-lived article no longer paints as uniformly "freshest"; and a revert that survives collapsing (most often the article's own true final revision) is never sampled as "the face" of a step when a non-flagged predecessor is available, is credited to the earlier byte-identical revision it restored when one exists, and otherwise gets an honest "↩ revert" label instead of being presented as fresh authorship.
