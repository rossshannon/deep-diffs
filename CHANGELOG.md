...
- `deep-diffs-wiki` CLI — renders a live Wikipedia article's revision history (MediaWiki Action API, rate-limit friendly) as a deep-diff report, including an author-coloured lens over the top contributors — the paper's original corpus, revisited. Wikitext stripping runs in linear time even on adversarial input (quadratic regex scans found in review and replaced with indexOf-based passes); vandalism/revert cycles are collapsed out of the revision timeline before sampling (History Flow's collapse rule, `--keep-reverts` to disable) and the Recency lens buckets by real calendar date rather than sample rank, so a heavily-vandalised or long-lived article no longer paints as uniformly "freshest".
